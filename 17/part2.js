...
while (toExplore.size() > 0) {
  ...
  if (m.length === 11 || x < 0 || y < 0 || x> mX || y > mY) continue

  if (m.length > 3 && x == mX && y == mY) {
    min = Math.min(min, d)
    if (d == min) path = p
  ...
  toExplore.push(...neighbors.filter(n => n[2] !== reverse[m.at(0)])
    .map(([nX, nY, nD]) => {
      const f = m.at(0) === nD ? 1 : 4

      const cost = Array(f).fill().map((_, i) => [x+nX*(i+1), y+nY*(i+1)]).reduce((s, [wx, wy]) => s + grid[`${wx}/${wy}`], 0)

      return [d + cost, x+f*nX, y+f*nY, m.at(0) === nD ? m + nD : nD.repeat(f), p+nD.repeat(f)]
    })
    .filter(n => !isNaN(n[0]))
  )
}